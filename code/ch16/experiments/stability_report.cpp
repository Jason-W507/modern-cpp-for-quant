...
int main() {
  ...
  const quant::ch16::TolerancePolicy tolerance_policy =
      quant::ch16::tolerance_from_budget(0.0, 2, 0.3);
  const bool tolerant =
      quant::ch16::almost_equal(0.1 + 0.2, 0.3,
  ...
  std::cout << "stability-ok exact-0.1+0.2=" << std::boolalpha << exact
            ...
            << "tolerance-source input-abs="
            << tolerance_policy.input_absolute_error
            << " operations=" << tolerance_policy.rounded_operations
            << " scale=" << tolerance_policy.scale
            << " abs=" << tolerance_policy.absolute
            << " rel=" << tolerance_policy.relative << '\n';
  return valid ? 0 : 2;
}
