...
\chapteropening{能区分配置、生成、构建和测试，用目标与依赖边组织库、应用、示例和测试，并解释属性传播。}{一个可从干净目录复现 Debug、Sanitized 与 Release 配置的 target-based CMake 快照。}{第 1、3、5 章；能编译多文件程序并理解头文件与链接。}

\section{问题：从源码树复现一个目标图}
...
根目录的 \path{CMakePresets.json} 把常用 Linux/WSL Debug、Linux/WSL Release 和 Windows MinGW Release 配置登记为版本化名称。CMake Presets 的版本化项目配置与用户本地覆盖有明确分工\autocite{cmakepresets}；因此它不保存开发者本机的绝对路径或密钥，这些个人覆盖应放进不提交的 \path{CMakeUserPresets.json}。在 WSL 中可直接运行：
