...
Conan 或 vcpkg 都可以把外部库解析为 CMake imported target，但可复现性还需要提交 manifest、版本约束和 lockfile/baseline。二者不应同时成为同一项目的默认依赖来源。GoogleTest 或 Catch2 适合 fixture、参数化测试和更清楚的失败差异；本书前半部继续使用无依赖测试程序展示“退出码就是最小测试协议”，后半部项目迁移框架时，四个公开行为边界与独立期望值仍保持不变。
