...
这里验证三条路径的结果一致；“并发更快”仍待独立测量。8 条数据远小于线程启动和调度成本，发布构建也不能让微小工作自动获得收益。立即练习：只把最后一条数量从 2 改为 3，先手算新 oracle 211150，再修改常量并要求三条路径同时通过。
...
立即练习：把线程作用域后的求和移动到作用域内部，解释它为何可能与写入并发；正确修复要恢复明确的 join 边，\texttt{sleep} 无法建立同步关系。
...
立即练习：在 \path{pipeline_compare.cpp} 的任务版移除显式 \texttt{std::launch::async}，打印任务与主调方线程 ID；仍须得到 \texttt{checksum-match=true}，并记录实现实际选择，但不要把一次观察写成标准保证。
