...
量化流水线常把行情批次分片计算，但完成判据必须落在结果和协议上，不能只写“开两个线程”。\path{code/ch15/experiments/pipeline_compare.cpp} 先用 8 条整数分行情建立顺序 oracle，再让两个 \texttt{std::jthread} 和两个 \texttt{std::async} 任务计算同一分区。独立手算为 201100 分；三条路径不一致时，任何吞吐比较立即作废。
...
线程启动后可以与创建者交错执行。两个潜在并发的冲突访问中，若至少一个为非原子操作，且二者之间没有 happens-before，程序就存在 data race；其行为未定义，后果远不止“偶尔少算”。两个原子操作即使没有普通的 happens-before，也不会仅因此构成 data race；这只说明单个原子对象的访问受原子语义约束，复合业务不变量仍需额外协议。先问谁拥有数据、谁只读、谁写入，再选同步工具。
...
停止请求不是强制杀线程；代码必须在阻塞点或循环边界协作检查。若任务正在无法取消的外部调用中，请求也不能保证立即结束。
...
\begin{enumerate}
  \item 只把最后一条数量从 2 改为 3，先手算新 oracle 211150，再修改常量并要求三条路径同时通过。
...
并发正确性来自所有权与明确同步，一次正确输出不足为证；背压和停止属于接口协议，不能降格为队列实现细节。先保留顺序 oracle，用 join、mutex、future 和 stop-aware 等待建立可推理边界，再用 sanitizer 与受控测量补证据，才能安全地把并发带入量化系统。
