\chapter{接口与值语义}
\index{value semantics}\index{interface}\index{strong invariant}

\chapteropening{能从失败场景推导订单、成交和组合接口，用值语义与组合装配组件，并在修改前维护价格、数量、现金和持仓不变量。}{一个从行情到订单、成交和组合快照的最小纵向闭环，以及两个公开行为测试边界。}{第 3--7 章；会使用类、异常、容器和 ranges。}
...
完成判据不只是“能打印”。零数量、非有限价格、超现金买入和超持仓卖出都必须在越过领域边界前失败；策略决策与成交/组合两个公开边界由独立行为测试保护；\path{code/ch08} 能从空构建目录独立配置。

\begin{remark}
本章为突出对象边界，暂用 \texttt{double} 表示价格和现金。这是教学简化，不是生产默认值。真实系统需要根据资产、币种、最小报价单位和舍入规则选择整数定点或专门金额类型；大数量与 \texttt{double} 相乘还可能丢失整数精度，\texttt{isfinite} 只能排除 NaN 和无穷，不能恢复已经舍入掉的低位。第 16 章会建立完整的表示策略。
\end{remark}
...
\pythoncompare{dataclass 可快速建模；C++ 构造函数与私有字段封闭合法状态。}{Python 属性寿命跟随对象图；C++ 值成员与宿主对象共同生灭。}{Python 依赖运行期验证或类型工具；C++ 接口类型先由编译器检查。}{两边的业务不变量都需显式拒绝；C++ 还能隐藏绕过入口。}
\end{pythonbridge}
...
成交/组合边界测试手算两笔成交。99 元买 25 股后现金 7525；101 元卖 10 股后现金 8535、持仓 15、权益 10050。它还验证错误代码与流动性不足都不成交、超卖与第二个代码都在修改前被拒绝。策略边界另外拒绝事件与快照代码不一致。测试只调用 \texttt{match}、\texttt{apply} 与 \texttt{snapshot}，不读取私有状态。本章快照明确限定一次运行一个代码；多资产总权益需要一组完整估值价格，留到第 17 章统一设计，不能只拿最后一个代码的价格冒充总权益。
...
最后一行必须与章首输出逐字一致。根项目还运行 8 项 ch08 检查：策略边界、成交/组合边界、主流水线、运行时替换、非法值、核心练习、错误覆盖诊断以及从空目录配置的快照。Windows 可选择 Visual Studio 生成器；多配置生成器的可执行文件通常位于 \texttt{Debug} 或 \texttt{Release} 子目录，接口与值语义规则不变。
...
\begin{enumerate}
  \item 为“订单数量大于行情可用数量时不成交”补一句用例。规则属于 \texttt{SimulatedExchange}，而非 \texttt{Portfolio}：撮合时已经同时拥有订单与行情，组合不应重新推断市场流动性。
  \item 在 \path{code/ch08/tests/test_invalid_values.cpp} 中把零数量改为负数，把 NaN 改为正无穷。两种构造仍应抛出 \texttt{invalid\_argument}，测试最终精确输出 \texttt{domain invariants ok}。
  \item 把阈值改为 98。测试中的 99 元事件应不再产生订单；先观察策略边界以“cheap event should create an order”失败，再同步修改用例或恢复阈值。失败消息证明测试保护的是行为而非内部实现。
  \item 运行 \path{code/ch08/exercises/sell_down_solution.cpp}，解释为何买入 25、卖出 10 后精确输出 \texttt{quantity=15 cash=8535.00 equity=10050.00}。再把卖出数量改为 26，程序应在产生输出前以状态不一致失败。
  \item 在 \texttt{HoldStrategy::on\_event} 参数末尾删除成员函数的 \texttt{const}，保留 \texttt{override}。预测并验证编译失败；恢复 \texttt{const} 后输出重新变为 \texttt{threshold=1 hold=0}。
\end{enumerate}
