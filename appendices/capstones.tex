...
\texttt{SequenceGate} 将消息序号 1、2、2、4 分类为两次 accepted、一次 duplicate/stale 和一次 gap，next expected 保持 3。\texttt{MarketReplay} 在这一层之上缓存未来消息；序号 3 到达后自动处理 3、再排空已缓存的 4。它仍不会静默跳过缺口，也没有假装实现交易所重传或快照协议。
...
\texttt{MarketReplay::apply} 是协议测试的公开边界（seam）。解码错误不推进序号；旧序号计为 duplicate/stale；未来序号进入有序缓存且保持 next expected；只有缺口闭合后才按连续序号进入订单簿。结果、buffered/recovered 计数与盘口都可从公开接口观察：
