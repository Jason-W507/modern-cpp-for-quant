...
\texttt{SequenceGate} 将消息序号 1、2、2、4 分类为两次 accepted、一次 duplicate/stale 和一次 gap，next expected 保持 3。\texttt{MarketReplay} 在这一层之上缓存未来消息；序号 3 到达后自动处理 3、再排空已缓存的 4。缓存同时限制允许的最大序号距离和待处理消息数，默认上限均为 1024；超过距离返回 \texttt{sequence\_gap\_exceeded}，容量耗尽时按 \texttt{reject\_newest} 策略返回 \texttt{pending\_overflow}，不会让恶意或损坏输入无限占用内存。它仍不会静默跳过缺口，也没有假装实现交易所重传或快照协议。
...
\texttt{MarketReplay::apply} 是协议测试的公开边界。解码错误不推进序号；旧序号计为 duplicate/stale；允许范围内的未来序号进入有序缓存并维持预期序号，只有缺口闭合后才按连续序号进入订单簿。
