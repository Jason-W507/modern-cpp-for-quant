...
\texttt{MarketReplay::apply} 是协议测试的公开边界（seam）。解码错误不推进序号；旧序号计为 duplicate/stale；允许范围内的未来序号进入有序缓存且保持 next expected；只有缺口闭合后才按连续序号进入订单簿。统计把 \texttt{sequence\_accepted}、\texttt{book\_accepted} 与 \texttt{book\_rejected} 分开，单次结果也会汇总连带恢复消息的接受或拒绝，避免把“序号已消费”误写成“订单已进入簿”。结果、buffered/recovered 计数、待处理数量与盘口都可从公开接口观察：
