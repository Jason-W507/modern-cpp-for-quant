...
固定回放先拒绝 3 字节坏消息，再处理序号 1、2、重复 2、缺口 4 和恢复到 3。消息 3 到达后，缓存中的 4 随即排空；验收结果是 sequence-accepted=4、book-accepted=4、duplicate=1、gap=1、buffered=1、recovered=1、decode=1、trades=2、next=5。另一组固定测试把距离限制设为 3、容量设为 2，分别冻结越距拒绝与满载拒绝；还让恢复消息在订单簿层失败，确认单次结果显式报告一次接受和一次拒绝。\path{test_spsc_replay.cpp} 进一步让单生产者线程通过容量 256 的 SPSC ring 交付 10000 条消息，消费线程全部回放，sequence-accepted、book-accepted 与 offered 必须相等。
