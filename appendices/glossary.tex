...
data race & 两个潜在并发的冲突访问中至少一个为非原子操作，且二者没有 happens-before；在 C++ 中属于 UB。\\
...
切片与 NumPy view & 区分拥有与借用 & \texttt{span}/view 不自动持有所有者；还须证明 dtype 与 stride \\
...
向量化与计时 & 先守住业务判定基准 & 数组表达式不保证零拷贝，一次 notebook 计时也不是 benchmark \\
\bottomrule
\end{tabularx}
