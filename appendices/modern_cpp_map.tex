...
这个实验刻意保留三个采用条件：输入长度相同且连续；尾部元素不能丢失；浮点归约次序变化只能在事先约定的误差界内接受。若要写入作品集，还应保存 \path{-fopt-info-vec} 或 \path{-Rpass=loop-vectorize} 报告、热点汇编、CPU 型号与成对原始样本。只有业务 oracle、生成代码和测量结果同时成立，SIMD 才从“用了新 API”升级为可审查的优化证据。
